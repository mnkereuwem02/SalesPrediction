...
operation = st.sidebar.selectbox("Select Operation", ("Sales Projection", "Optimize Pricing"))

# Main App Title
st.title("Sales Forecasting & Price Optimization Tool")

if operation == "Sales Projection":
    st.header("Predict Sales Quantity")
    st.write("Enter the required parameters to predict your sales quantity.")
    


    col1, col2 = st.columns(2)
    with col1:
        price_x = st.number_input("Product's Price", min_value=0.0, value=300.0, step=1.0)
        price_y = st.number_input("Competitor's Price", min_value=0.0, value=100.0, step=1.0)
        order_year = st.number_input("Order Year", min_value=2000, max_value=2100, value=2025, step=1)
 ...
 'Furniture': ['Tables', 'Chairs', 'Sofas', 'Bookcases']}
    ...
    st.write("Input your constant parameters, with a price range from your competitor to determine the pricing.")
